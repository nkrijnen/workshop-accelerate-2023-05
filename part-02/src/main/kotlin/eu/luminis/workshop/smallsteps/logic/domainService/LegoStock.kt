...
data class LegoStock(
    ...
    private fun LegoBox.requireValidMissingLegoParts() {
        require(missingParts.isNotEmpty()) { "No missing parts specified" }
        require(missingParts.hasNoneWithCountZero()) { "You must specify how many parts are missing" }
        ...
        require(missingParts.partsPresentIn(LegoParts(expectedParts))) {
            "Some parts reported missing don't belong to lego set $legoSet"
        }
        require(missingParts.hasNoMoreThan(LegoParts(expectedParts))) {
